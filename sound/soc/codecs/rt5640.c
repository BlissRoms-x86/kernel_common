...
/*
 * Note we do not toggle the RT5640_MIC1_OVCD_EN bit in these 2 functions,
...
static void rt5640_disable_micbias1_ovcd(struct snd_soc_codec *codec)
{
	...
	snd_soc_dapm_mutex_lock(dapm);
	if (rt5640->jack_data->clk)
		snd_soc_dapm_disable_pin_unlocked(dapm, rt5640->jack_data->clk);
...
static void rt5640_start_button_press_work(struct rt5640_priv *rt5640)
{
	rt5640->poll_count = 0;
	...
	rt5640_clear_micbias1_ovcd(rt5640->codec);
...
static void rt5640_jack_work(struct work_struct *work)
{
	...
	if (!rt5640_jack_inserted(codec)) {
		 ...
		 */
		rt5640_disable_micbias1_ovcd_irq(rt5640);
		rt5640_start_button_press_work(rt5640);
			   ...
			   struct snd_soc_jack *jack, void *data)
{
	...
	rt5640->jack = jack;
	rt5640->jack_data = data;

	/* Select JD-source */
	snd_soc_update_bits(codec, RT5640_JD_CTRL,
			    RT5640_JD_MASK, rt5640->jack_data->pin);
	 ...
	 * 10% of the time, but as we poll the ovcd status bit we might hit that
