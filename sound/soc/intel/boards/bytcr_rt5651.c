 ...
 */
static int byt_rt5651_add_codec_device_props(const char *i2c_name)
{
	struct property_entry props[MAX_NO_PROPS] = {};
	struct device *i2c_dev;
	int ret, cnt = 0;

	i2c_dev = bus_find_device_by_name(&i2c_bus_type, NULL, i2c_name);
	if (!i2c_dev)
		return -EPROBE_DEFER;
...
static int snd_byt_rt5651_mc_probe(struct platform_device *pdev)
{
	...
	ret_val = byt_rt5651_add_codec_device_props(i2c_name);
	if (ret_val)
		return ret_val;
