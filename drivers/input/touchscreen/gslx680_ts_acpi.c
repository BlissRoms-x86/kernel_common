...
static int gsl_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	bool acpipower;
#ifdef CONFIG_ACPI
	// get touchscreen prop from ACPI
	...
	if(adev) {
		dev_warn(&client->dev, "Getting ACPI property for the ts\n");
		status = acpi_evaluate_object(adev->handle, "PRP0", NULL, &buffer);
		if(!ACPI_FAILURE(status)) {
			obj=buffer.pointer;
			if(obj && obj->package.count && obj->type == ACPI_TYPE_PACKAGE) {
				elem = &obj->package.elements[0]; // FW Name
				if(elem->type == ACPI_TYPE_STRING) {
					dev_warn(&client->dev, "FW: %s\n", elem->string.pointer);
				}else{
					dev_warn(&client->dev, "FW: ?\n");
				}
				elem = &obj->package.elements[1]; // X Max
				if(elem->type == ACPI_TYPE_INTEGER) {
					dev_warn(&client->dev, "XMax: %lld\n", elem->integer.value);
				}else{
					dev_warn(&client->dev, "XMax: ?\n");
				}
				elem = &obj->package.elements[2]; // Y Max
				if(elem->type == ACPI_TYPE_INTEGER) {
					dev_warn(&client->dev, "YMax: %lld\n", elem->integer.value);
				}else{
					dev_warn(&client->dev, "YMax: ?\n");
				}
			}else{
				dev_warn(&client->dev, "ERROR: INVALID ACPI OBJ\n");
			}
		}else{
			dev_warn(&client->dev, "ERROR: ACPI FAILURE\n");
		}
	}else{
		dev_warn(&client->dev, "ERROR: NO ACPI DEV\n");
	}
#endif
