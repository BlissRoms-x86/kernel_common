...
static void ipts_mei_cl_event_cb(struct mei_cl_device *cldev,
					u32 events,
					void *context)
{
	ipts_info_t *ipts = (ipts_info_t *)context;
	ssize_t msg_len;
	...
	if (test_bit(MEI_CL_EVENT_RX, (unsigned long *)&events)) {
		...
		ipts_dbg(ipts, "ipts got non-event rx\n");
	}
			...
			const struct mei_cl_device_id *id)
{
	...
	ret = mei_cldev_register_event_cb(cldev, BIT(MEI_CL_EVENT_RX),
					ipts_mei_cl_event_cb, ipts);
	if (ret < 0) {
		pr_err("cannot register IPTS event callback\n");
		goto free_ipts;
	}
